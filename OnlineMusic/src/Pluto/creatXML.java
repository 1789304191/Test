...
public class creatXML {
	...
	public void bulidXML(String [] id,HttpServletRequest request,String userName) throws IOException, JDOMException, SQLException {
		...
		*/
		...
		File cacheDir = new File(request.getSession().getServletContext().getRealPath("/player/xml/" + user_id + ".xml"));//设置目录参数 
		File path=new File(request.getSession().getServletContext().getRealPath("/player/xml/"));
		if(!path.exists()){
			path.mkdir();
			cacheDir.createNewFile();//新建目录 
		}
